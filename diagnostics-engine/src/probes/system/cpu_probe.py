...
def run(config: ProbeConfig, job_id: str) -> Evidence:
    ...
    try:
        ...
        try:
            ...
            load1 = load5 = load15 = None  # not available on all platforms
        ...
        if overall_pct >= config.cpu_crit_pct:
            ...
            message = f"CPU usage {overall_pct:.1f}% >= critical threshold {config.cpu_crit_pct}%"
        elif overall_pct >= config.cpu_warn_pct:
            status = ProbeStatus.DEGRADED
            message = f"CPU usage {overall_pct:.1f}% >= warning threshold {config.cpu_warn_pct}%"
        else:
            status = ProbeStatus.OK
            message = f"CPU usage normal ({overall_pct:.1f}%)"

        return Evidence(
            probe_type=ProbeType.CPU, target=config.target, status=status,
            latency_ms=latency_ms, raw=raw, message=message, job_id=job_id,
        )
    except Exception as exc:  # noqa: BLE001
        return Evidence.error_result(ProbeType.CPU, config.target, exc, job_id=job_id)
