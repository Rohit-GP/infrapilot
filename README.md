...
1. **Repo skeleton** (this commit) — stub every service so the pipeline can
...
3. **Redis Streams** — probing engine publishes evidence to a stream instead
   of stdout.
4. **Backend orchestration (Spring Boot)** — job lifecycle, REST + STOMP
...
## Quickstart (diagnostics engine only, for now)
...
See `diagnostics-engine/README.md` for details.
